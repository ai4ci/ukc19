...
variants = readr::read_tsv(
  raw_lineages,
  na = c("Lineage data suppressed", "Unassigned")
)

lineages = .cache_download(
...
lineage_key_df = dplyr::tibble(
  prefix = names(lineage_keys),
  expanded = unlist(unname(lineage_keys))
)
recombinant_key_df = dplyr::tibble(
  prefix = names(recombinants),
  expanded = names(recombinants)
)
...
variants2 = variants %>%
  dplyr::mutate(
    ...
    suffix = Lineage %>%
      stringr::str_sub(start = stringr::str_length(prefix) + 1)
  ) %>%
  dplyr::left_join(lineage_key_df, by = "prefix") %>%
  dplyr::mutate(
    full_tree = dplyr::case_when(
...
covid_variants = variants2 %>%
  ...
  dplyr::group_by(date, class, who_class) %>%
...
if (interactive()) {
  usethis::use_data(covid_variants, overwrite = TRUE)
}


covid_variants_ltla = variants2 %>%
  dplyr::filter(lineage_class != "Unclassified") %>%
  dplyr::inner_join(
    geography %>% filter(codeType == "LAD19"),
    by = c("LTLA" = "code")
  ) %>%
  dplyr::select(
    date = WeekEndDate,
  ...
  dplyr::group_by(date, name, codeType, code, class, who_class) %>%
  dplyr::summarise(count = sum(count, na.rm = TRUE)) %>%
  dplyr::group_by(date, name, codeType, code) %>%
  dplyr::mutate(
    denom = sum(count)
  ) %>%
  dplyr::ungroup()
...
if (interactive()) {
  usethis::use_data(covid_variants_ltla, overwrite = TRUE)
}
