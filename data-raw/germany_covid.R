...
germany_demographics = covid_count_3 %>%
  ...
  dplyr::summarise(
    population = as.integer(round(mean(pop, na.rm = TRUE))),
    .groups = "keep"
  )


if (interactive()) {
  interfacer::use_dataframe(germany_demographics)
}
...
germany_covid = covid_count_1 %>%
  ...
  dplyr::mutate(
    time = as.time_period(date, unit = "1 week", start_date = "2019-12-30")
  ) %>%
...
if (interactive()) {
  interfacer::use_dataframe(germany_covid)
}
